import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/43d313-part-7/43d313-part-7.ot2.apiv2.py"
...
metadata = {
    ...
    Post-PCR Instrument: RNA POST PCR 3 : Post-Indexing Cleanup''',
...
def run(ctx):
    ...
    # keep unused item on deck to avoid unnecessary changes between scripts
    p300m.transfer(0, reagents.columns()[0][0], reagents.columns()[0][0])

    # reservoir for beads and tris
    reservoir = ctx.load_labware('nest_12_reservoir_15ml', '5', 'Reservoir')

    beads = reservoir.wells()[0]
    liq_volume([beads], 5760)
    ...
    for column in mag_plate.columns()[:num_cols]:
        ...
        p300m.aspirate(48, beads.bottom(clearance_reservoir), rate=0.5)
        ctx.delay(seconds=1)
        p300m.dispense(48, column[0])
        p300m.mix(10, 70)
        p300m.drop_tip()
    ...
    # transfer eluted sample to new plate
    for index, column in enumerate(mag_plate.columns()[:num_cols]):
    ...
    ctx.pause(
     '''Post-indexing cleanup protocol complete.
     Proceed to library QC protocol''')
    ...
    filename = f"protocols/detailed_action_json/43d313-part-7.json"
    output_data = {
