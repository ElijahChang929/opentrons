import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/76c562/76c562.ot2.apiv2.py"
...
def run(ctx):
    ...
    for mount in pipettes.keys():
        if 'instrument' in pipettes[mount]:
            ...
            if start_slot not in allowable_slots:
                raise Exception(f'Start Slot for {mount} pipette must be in  {allowable_slots}.')
    ...
    def pick_up(pip):
        if tip_log[pip]['count'] == tip_log[pip]['max']:
            ctx.pause('Please refill {}µl tipracks before  resuming.'.format(pip.max_volume))
            pip.reset_tipracks()
            tip_log[pip]['count'] = 0
    ...
    filename = f"protocols/detailed_action_json/76c562.json"
    output_data = {
