import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/10bf60-station-B/extraction.ot2.apiv2.py"
...
def run(ctx):
    ...
    [num_samples, starting_vol, binding_buffer_vol, wash1_vol, wash2_vol,
     ...
     flash] = get_values(  # noqa: F821
        ...
        'wash2_vol', 'elution_vol', 'mix_reps', 'settling_time', 'park_tips',
        'tip_track', 'flash')
    ...
    waste = [
        ...
        ctx.load_labware(
            'nest_12_reservoir_15ml', '11', 'Liquid Waste').wells()]
    ...
    def remove_supernatant(vol, park=False):
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for _ in range(num_trans):
                waste_loc = _waste_track(vol_per_trans)
    ...
    def elute(vol, park=True):
        ...
        if magdeck.status == 'enagaged':
            magdeck.disengage()
        for i, (m, e, spot) in enumerate(zip(mag_samples_m, elution_samples_m,
                                             parking_spots)):
            _pick_up(m300)
            side = 1 if i % 2 == 0 else -1
            loc = m.bottom(0.5).move(Point(x=side*2))
            ...
            m300.transfer(50, loc, e.bottom(2), new_tip='never')
            m300.blow_out(e.bottom(10))
            m300.air_gap(20)
            if park:
                m300.drop_tip(spot)
            else:
        ...
        for i, (m, e, spot) in enumerate(zip(mag_samples_m, elution_samples_m,
            ...
            if park:
                ...
                _pick_up(m300)
            m300.transfer(vol, e.bottom(1), m, mix_before=(mix_reps, 0.8*vol),
                          new_tip='never')
            m300.blow_out(m.bottom(15))
            m300.air_gap(20)
            if park:
                m300.drop_tip(spot)
            else:
        ...
        ctx.pause('Replace strips on temperature module (slot 1) with fresh  strips before resuming.')
        magdeck.engage(height=MAG_HEIGHT)
        ctx.delay(minutes=settling_time, msg='Incubating on MagDeck for  ' + str(settling_time) + ' minutes.')
    ...
    filename = f"protocols/detailed_action_json/10bf60-station-B.json"
    output_data = {
