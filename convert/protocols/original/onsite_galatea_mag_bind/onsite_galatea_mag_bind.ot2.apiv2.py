import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/onsite_galatea_mag_bind/onsite_galatea_mag_bind.ot2.apiv2.py"
...
metadata = {
    'protocolName': 'Galatea Mag-Bind® Blood & Tissue DNA HDQ 96 Kit',
...
def run(ctx):
    ...
    filename = f"protocols/detailed_action_json/onsite_galatea_mag_bind.json"
    output_data = {
...
#    tempdeck.set_temperature(4)
    ...
    def remove_supernatant(vol, park=False):
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            loc = m.bottom(1).move(Point(x=side*1.3))
    ...
    def resuspend_pellet(well, pip, mvol=150, reps=4):
        """
        ...
        top = [
            well.bottom().move(types.Point(x=-1, y=1, z=2)),
            well.bottom().move(types.Point(x=1, y=1, z=2))
        ]
        bottom = [
            well.bottom().move(types.Point(x=-1, y=-1, z=2)),
    ...
    def bind(vol, park=True):
        ...
        magdeck.engage(height_from_base=3.5)
