import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/0c3e45/liquid_transfer.ot2.apiv2.py"
...
def run(ctx):
    ...
    def find_tip(pip, loc=None):
        if tip_log[pip]['count'] == tip_log[pip]['max'] and not loc:
            ctx.pause(f'Replace {str(pip.max_volume)}µl tipracks before  resuming.')
            pip.reset_tipracks()
            tip_log[pip]['count'] = 0
        if loc:
            tip = loc
    ...
    ctx.home()
    ctx.pause(f'Ensure tips are placed in column {column_ind+1} of tiprack on  slot 11 before resuming.')
