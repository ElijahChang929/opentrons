import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/0cded6-cleanup/cleanup.ot2.apiv2.py"
...
def run(ctx):
    ...
    tag1_plate = magdeck.load_labware(
            'biorad_96_wellplate_200ul_pcr', 'TAG1 plate')
    ...
    st2 = reagent_plate.rows()[0][8]
    twb = reservoir.rows()[0][:(math.ceil(num_cols/6))]
    liquid_trash = [
        well.top()
    ...
    filename = f"protocols/detailed_action_json/0cded6-cleanup.json"
    output_data = {
...
MagDeck for {time_settling} minutes.')
    ...
    for d in mag_samples:
        pick_up(m20)
        m20.aspirate(vol_st2, st2.bottom(0.5))
        slow_withdraw(m20, st2)
        m20.dispense(m20.current_volume, d.bottom(2))
        m20.mix(mixreps, 20, d.bottom(2))
        slow_withdraw(m20, d)
        if TEST_MODE_DROP:
            m20.return_tip()
        else:
...
#     ctx.pause('\n\n\n\nSeal and shake at 1600 rpm for 1 minute. Incubate \
# at room temperature for 5 minutes. Centrifuge at 500 × g for 1 minute. \
