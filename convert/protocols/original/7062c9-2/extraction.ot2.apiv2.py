import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/7062c9-2/extraction.ot2.apiv2.py"
...
def run(ctx):
    ...
    def wash(vol, source, change_tips_for_samples=True, mix_reps=mix_reps,
        ...
        ctx.delay(minutes=bead_settling_time, msg='Incubating on MagDeck for  ' + str(bead_settling_time) + ' minutes.')
    ...
    def elute(vol, park=True):
        ...
        ctx.delay(minutes=temp_time, msg=f'Incubating at 85C for {temp_time}  minutes')
    ...
    # prewash
    magdeck.engage(mag_height)
    ctx.delay(minutes=bead_settling_time, msg=f'Beads separating for  {bead_settling_time} minutes.')
    ...
    filename = f"protocols/detailed_action_json/7062c9-2.json"
    output_data = {
