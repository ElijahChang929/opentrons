import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/78d33c-part-4/78d33c-part-4.ot2.apiv2.py"
...
metadata = {
    ...
    Post-PCR Instrument: DNA-POST-PCR-1 Purification Post Indexing''',
...
def run(ctx):
    ...
    depletion_mm = reagents.columns()[0]
    liq_volume(depletion_mm, 27 + num_cols*15)
    ...
    ctx.comment("STEP - Depletion Master Mix")
    ...
    for index, column in enumerate(plate4deg.columns()[:num_cols]):
        ...
        depletion_mm[0].liq_vol -= 15
        ht = liq_height(
         depletion_mm[0]) - 3 if liq_height(depletion_mm[0]) - 3 > 1 else 1
        p20m.transfer(
         15, depletion_mm[0].bottom(ht), column[0], new_tip='never')
        p20m.drop_tip()
    ...
    ctx.pause(
     '''Continue to off-deck thermocycler steps and
     DNA-POST-PCR-2 Post Indexing protocol.''')
    ...
    filename = f"protocols/detailed_action_json/78d33c-part-4.json"
    output_data = {
