import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/e9ff8d_part4/e9ff8d_part4.ot2.apiv2.py"
...
def run(ctx):
    ...
    for _ in range(15):
        m300.aspirate(25, mix_loc, rate=0.5)
        ctx.delay(seconds=1)
        m300.dispense(25, mix_loc, rate=0.5)
        ctx.delay(seconds=1)
    m300.move_to(samples.top())
    ctx.delay(seconds=5)
    blowout(samples)
    m300.drop_tip()
    ...
    filename = f"protocols/detailed_action_json/e9ff8d_part4.json"
    output_data = {
