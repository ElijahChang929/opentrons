import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/00a214-protocol-5/00a214-protocol-5.ot2.apiv2.py"

import csv
import os

metadata = {
    'protocolName': 'Primary Staining',
...
def run(protocol):
    ...
    tips300 = [protocol.load_labware(
        'opentrons_96_tiprack_300ul', s) for s in ['1', '4']]

    tipsA = [tip for rack in tips300 for tip in rack.rows()[0]]
    ...
    def pick_up():
        ...
        if tip300count == 24:
            ...
            p300.reset_tipracks()
            tip300count = 0
        p300.pick_up_tip(tipsA[tip300count])
        tip300count += 1
    ...
    for idx, (plate, res_src) in enumerate(zip(plates, pbsDapi)):
        ...
        protocol.comment(f'Adding PBST to plate {idx+1}...')
        for well in wells:
            p300.aspirate(140, res_src)
            p300.dispense(140, well.bottom(disp_height))
        p300.drop_tip()

    protocol.comment('Cells should be in PBST for at least 12 minutes...')
    protocol.pause('Click RESUME when ready')

    for idx, (plate, res_src) in enumerate(zip(plates, pbs3)):
        wells = plate.rows()[0]
        protocol.comment(f'Removing PBST from plate {idx+1}...')
        pick_up()
        for well in wells:
            p300.aspirate(140, well.bottom(asp_height))
            p300.dispense(140, waste)
        protocol.comment(f'Adding Blocking Buffer to plate {idx+1}...')
        for well in wells:
            p300.aspirate(140, res_src)
            p300.dispense(140, well.bottom(disp_height))
        p300.drop_tip()

    protocol.comment('Cells should be in buffer for at least 45 minutes...')
    protocol.pause('Click RESUME when ready')

    for idx, (plate, res_src) in enumerate(zip(plates, pbs4)):
        wells = plate.rows()[0]
        protocol.comment(f'Removing buffer from plate {idx+1}...')
        pick_up()
        for well in wells:
            p300.aspirate(140, well.bottom(asp_height))
            p300.dispense(140, waste)
        protocol.comment(f'Adding Primary Antibody to plate {idx+1}...')
        for well in wells:
            p300.aspirate(100, res_src)
            p300.dispense(100, well.bottom(disp_height))
        p300.drop_tip()
    ...
    filename = f"protocols/detailed_action_json/00a214-protocol-5.json"
    output_data = {
