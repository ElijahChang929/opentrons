import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep Potential/opentrons/convert/protocols/original/macherey-nagel-nucleomag-size-select/macherey-nagel-nucleomag-size-select.ot2.apiv2.py"
...
def run(ctx):
    ...
    Variable definition
    '''
    [num_samples, starting_vol, elution_vol, NGS_bead_ratio1, NGS_bead_ratio2,
     wash1_vol, wash2_vol, mix_reps_bind, mix_reps_wash,
     mix_reps_elu, sep_time, dry_time, tip_track] = get_values(  # noqa: F821
     'num_samples', 'starting_vol', 'elution_vol', 'NGS_bead_ratio1',
     'NGS_bead_ratio2', 'wash1_vol', 'wash2_vol', 'mix_reps_bind',
     'mix_reps_wash', 'mix_reps_elu', 'sep_time', 'dry_time', 'tip_track')

    # values = [48, 100, 30, 0.4, 0.6, 200, 200, 0, 15, 10, 20, 5, 10, False]
    ...
    def remove_supernatant(vol, reuse=False, restore=False, last=False):
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for _ in range(num_trans):
                ...
                m300.transfer(
                    vol_per_trans, loc,
                    liquid_waste, new_tip='never', air_gap=20)
    ...
    def remove_small_fragments(bead_vol):
        ...
        remove_supernatant(
            supernatant_Bind2+5, reuse=False, restore=False, last=False)
    ...
    def remove_big_fragments(bead_vol):
        ...
        for i, (well, spot) in enumerate(zip(mag_samples_lm, parking_spots)):
            _pick_up(m300)
            if m300.current_volume > 0:
                m300.dispense(m300.current_volume, NGS_Beads.top())
            m300.flow_rate.aspirate = 10
        ...
        for j, (left, right, spot) in enumerate(
                zip(mag_samples_lm, mag_samples_m, parking_spots)):
            _pick_up(m300)
            m300.transfer(
                supernatant_Bind1+5, left.bottom(0.2), right.bottom(4),
                air_gap=10, new_tip='never')
            _drop(m300)
        m300.flow_rate.aspirate = 100
        m300.flow_rate.dispense = 150

    def wash(vol, source, mix_reps=mix_reps_wash, reuse=True, restore=True,
             resuspend=True, last=False):
        """
        ...
        if reuse:
            ...
            for i, (m) in enumerate(mag_samples_m):
                ...
                for n in range(num_trans):
                    ...
                    m300.transfer(
                        vol_per_trans, src.bottom(bottom_tolerance), m.top(),
    ...
    filename = f"protocols/detailed_action_json/macherey-nagel-nucleomag-size-select.json"
    output_data = {
