import builtins
builtins.event_logs = []
__protocol_file__ = r"/Users/guangxinzhang/Documents/Deep_Potential/opentrons/convert/protocols/original/1b2788/1b2788.ot2.apiv2.py"
...
def run(ctx):
    ...
    for index, s_col in enumerate(samples):
        ...
        for _ in range(2):
            ...
            m300.touch_tip(speed=40, radius=0.9)
            m300.dispense(prewash_buff_vol/2, waste, rate=0.4)
            m300.air_gap(airgap)
            m300.touch_tip(speed=40, radius=0.9)
    ...
    for i in range(2):
        ...
        for index, s_col in enumerate(samples):
            ...
            for _ in range(2):
                ...
                m300.touch_tip(speed=40, radius=0.9)
                m300.dispense(gdna_buff_vol/2, waste)
                m300.air_gap(airgap)
                m300.touch_tip(speed=40, radius=0.9)
            if i == 0:
                m300.drop_tip(ctx.loaded_labwares[8].rows()[0][index])
    ...
    filename = f"protocols/detailed_action_json/1b2788.json"
    output_data = {
