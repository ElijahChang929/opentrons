...
def run(ctx):
    ...
    for i in range(TOTAL_COl):
        ...
        p300.aspirate(MEDIUM_VOL*1.2, final.bottom(z=0.2).move(Point(x=-2.5)), rate = 10)
        p300.dispense(MEDIUM_VOL*1.2, waste.top(z=-5), rate = 10)
        p300.blow_out
        p300.drop_tip()
    ...
    for j in range(TOTAL_COl):
        final = cells_all[j]
        p300.aspirate(PBS_VOL, pbs.bottom(z=0.5), rate = 10)
        p300.air_gap(20)
        p300.dispense(PBS_VOL+20, final.top(z=-2), rate = 10)
        p300.blow_out()
        p300.touch_tip()
    p300.drop_tip()
    ...
    for l in range(TOTAL_COl):
        final = cells_all[l]
        p300.aspirate(LYSIS_VOL, lysis.bottom(z=0.5), rate = 10)
        ctx.delay(seconds=2)
        p300.dispense(LYSIS_VOL, final.bottom(z=5), rate = 10)
        ctx.delay(seconds=2)
        p300.touch_tip()
    p300.drop_tip()
    ...
    for m in range(TOTAL_COl):
        p300.pick_up_tip()
        final = cells_all[m]
        p300.aspirate(LUC_VOL, luciferase.bottom(z=0.5), rate = 10)
        ctx.delay(seconds=2)
        p300.air_gap(20)
        p300.dispense(LUC_VOL+20, final.top(z=-0.5), rate = 10)
        p300.mix(3, 75, final.bottom(z=0.5), rate = 10)
